...
RMSE_max = sqrt(mean((X_centre(:) - repmat(individu_moyen',n,1)).^2));

% Composantes principales des données d'apprentissage
C = X_centre*vectPropresSigma;

for q = 1:n-1 %0
    qPremCompPrinc = C(:,1:q);		            % q premières composantes principales
    qPremEigenfaces = valPropres(:,1:q);		% q premières eigenfaces
    X_reconstruit = X_centre*vectPropresSigma(:,1:q)*(vectPropresSigma(:,1:q)') + individu_moyen;
    figure(1);
